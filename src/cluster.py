...
def get_query(best =  False, id = False):
    ...
    if best:
        return { 'bedrooms': bedrooms, 'bathrooms': bathrooms, 'accommodates': accommodates, 'latitude': latitude, 'longitude': longitude}
...
def best_amenities():
    amenity_df = get_data()
    query = get_query(best = True, id = '10709107')
...
def get_query_from_id(id, best = False):
    amenity_df = get_data(id = True)
    id_property = amenity_df.query('airbnb_property_id == @id')


    if best:
        return {'bedrooms': id_property['bedrooms'], 'bathrooms': id_property['bathrooms'], 'accommodates': id_property['accommodates'], 'latitude': id_property['latitude'], 'longitude': id_property['longitude']}

    else:
        amenity = input('Amenity')
        return {'bedrooms': id_property['bedrooms'], 'bathrooms': id_property['bathrooms'], 'accommodates': id_property['accommodates'], 'latitude': id_property['latitude'], 'longitude': id_property['longitude'], 'amenity': amenity}

    import pdb; pdb.set_trace()

rev_potentials = best_amenities()
