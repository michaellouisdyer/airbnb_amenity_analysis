...
class PropertyAnalyzer(object):
    ...
    def get_comps(self):
        ...
        # create df from list for consistency
    ...
    def load_comps(self, comps, my_property, amenities):
        """Load variables from saved variables"""
        self.comps = comps
    ...
    def find_revenue_potential(self, amenity_x, scaling=True):
        ...
        query_df = self.my_property[['bedrooms', 'bathrooms',
                                     'accommodates', 'latitude', 'longitude']].astype('float')

        # Comp dataframe to find neighbors in
        df = self.comps[['rev_pot', 'bedrooms', 'bathrooms', 'accommodates',
                         'latitude', 'longitude', amenity_x]].astype('float')
    ...
    def spacy_tokenizer(self, doc):
        ...
        tokens = parser(doc)
        tokens = [tok.lemma_.lower().strip() if tok.lemma_ !=
                  "-PRON-" else tok.lower_ for tok in tokens]
    ...
    def NLP(self):
        ...
        vec = TfidfVectorizer(tokenizer=self.spacy_tokenizer,
                              max_features=15, max_df=1.0, ngram_range=(1, 1))
        ...
        coefficients_df = pd.DataFrame.from_dict(
            dict(zip(vec.get_feature_names(), ls.coef_)), orient='index').sort_values(by=0)
